...
considerably more optimistic.  \textbf{Caveat:} the power-law fit is
based on only 7 points over $n \in [15, 37]$; extrapolation to
$n > 100$ is speculative and the fitted exponent may change.
...
\paragraph{Interpretation.}
\begin{enumerate}[leftmargin=*]
  \item \textbf{Gradient Boosting ranks first}, with a
        20\,mg\,g$^{-1}$ advantage over Random Forest on CV RMSE and
        7.5\,mg\,g$^{-1}$ on 10-trial RMSE.
        However, a paired Wilcoxon signed-rank test on the 10 per-trial
        RMSEs yields $W=22$, $p=0.313$ (one-sided), meaning the
        difference is \textbf{not statistically significant}.  The MAE
        comparison is also non-significant ($\Delta = -14.6$, $p = 0.116$).
        As with Dataset~4, both tree-based models perform comparably.
  \item \textbf{GB is significantly better than SVR, Ridge, and Lasso}
        (Wilcoxon $p \leq 0.002$ for all three), confirming the
        non-linearity of the feature--target relationship.
...
\caption{Per-trial metrics for the HPO-selected Gradient Boosting model on
  ...
  MAE is included as a skew-robust complement to RMSE.}
...
Seed & $n_\text{train}$ & $n_\text{test}$ & Test R$^2$ & Test RMSE & Test MAE & Frac.\ high \\
\midrule
521  & 32 & 8 &  0.948 &  52.7 &  45.3 & 0.00 \\
737  & 32 & 8 & $-0.362$ & 144.3 &  94.9 & 0.00 \\
740  & 32 & 8 &  0.760 & 311.6 & 227.5 & 0.25 \\
660  & 32 & 8 &  0.888 & 182.7 & 135.8 & 0.12 \\
411  & 32 & 8 &  0.925 & 130.6 &  84.8 & 0.12 \\
678  & 32 & 8 &  0.705 & 170.4 & 153.5 & 0.00 \\
626  & 32 & 8 &  0.792 & 293.4 & 200.3 & 0.12 \\
513  & 32 & 8 &  0.849 & 207.0 & 162.5 & 0.12 \\
859  & 32 & 8 &  0.543 & 214.0 & 164.9 & 0.00 \\
136  & 32 & 8 &  0.882 & 171.4 & 126.8 & 0.12 \\
\midrule
\textbf{Mean} & & & \textbf{0.693} & \textbf{187.8} & \textbf{139.6} & \\
\textbf{Std}  & & & \textbf{0.411} & \textbf{75.7}  & \textbf{54.6}  & \\
\textbf{CV}   & & & 0.593          & 0.403           & 0.391           & \\
\bottomrule
\end{tabular}
\end{table}
...
$0.123 \pm 0.073$ and to the theoretical minimum.  As with Dataset~4,
low Jaccard confirms low sample overlap between folds but does \emph{not}
imply independence of prediction errors.
...
\paragraph{Summary.}
LOO-CV yields RMSE~$= 220.8$\,mg\,g$^{-1}$ (MAE~$= 149.5$) and $R^2 = 0.838$ in
original space — a higher RMSE than the 10-trial mean (187.8) but
with a notably better $R^2$ (0.838 vs.\ 0.693).  As discussed for
Dataset~4, the LOO estimator is deterministic but has \emph{high
estimator variance} due to correlated folds; its $R^2$ should be
interpreted cautiously with single-sample test folds.

\paragraph{Caveat.}
The LOO-CV and 10-trial RMSEs are not directly comparable (different
test-fold sizes and variance properties).  The 10-fold CV RMSE of
$221.3 \pm 153.6$ and repeated 5$\times$10 CV RMSE of
$228.1 \pm 136.3$ provide a third reference that is broadly consistent
with both.
...
% -----------------------------------------------------------------------
\subsection{Residual Diagnostics — Dataset~5}
\label{sec:ds5_residual_diagnostics}
% -----------------------------------------------------------------------

\begin{table}[h]
\centering
\caption{Heteroscedasticity tests — Dataset~5 (LOO residuals).}
\label{tab:ds5_hetero}
\begin{tabular}{lrrl}
\toprule
Test & $\rho$ & $p$ & Interpretation \\
\midrule
Spearman($|r|$, $\hat{y}$) & 0.354 & 0.025 & Significant \\
Spearman($|r|$, $y$)       & 0.174 & 0.281 & Not significant \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Finding:}  Heteroscedasticity is \emph{weaker} in
Dataset~5 than in Dataset~4 ($\rho = 0.174$ vs.\ 0.327 for $|r|$ vs.\
$y$).  This suggests the revised feature set produces more homogeneous
errors across the target range, though heteroscedasticity with respect
to predicted values remains significant.

\begin{table}[h]
\centering
\caption{Per-quartile residual analysis — Dataset~5 (LOO predictions).}
\label{tab:ds5_quartile_bias}
\small
\begin{tabular}{lrrrl}
\toprule
Quartile & Mean bias & MAE & Target range (mg\,g$^{-1}$) & Interpretation \\
\midrule
Q1 (lowest)  & $-145.3$ & 145.3 & [13, 95]     & Systematic over-prediction \\
Q2           & $+6.3$   &  42.3 & [96, 275]    & Well-calibrated \\
Q3           & $-5.3$   & 123.6 & [285, 526]   & Well-calibrated \\
Q4 (highest) & $+116.6$ & 286.6 & [532, 2251]  & Severe under-prediction \\
\bottomrule
\end{tabular}
\end{table}

\noindent Dataset~5 shows better calibration in Q2--Q3 (bias $\approx 0$)
compared with Dataset~4's persistent over-prediction.  However, Q1 and
Q4 still show strong regression-to-the-mean bias, with Q4 MAE (287) being
2.0$\times$ Q1 MAE (145).

\paragraph{Complementary metrics.}  Residual normality test:
Shapiro--Wilk $p = 0.009$ (non-normal).  MAE below median ($\leq 280$):
93.8; MAE above median: 205.1 (ratio 2.19).  MAPE: 135.2\,\%.
...
\caption{Gradient Boosting feature importances — Dataset~5
  ...
  Permutation importance (30 repeats) is included as a less biased
  alternative to impurity-based importance.}
\label{tab:ds5_importance}
\begin{tabular}{lrr}
\toprule
Feature & Impurity & Permutation ($\pm$~std) \\
\midrule
SSA (m$^2$\,g$^{-1}$)            & 0.3911 & 0.3514 $\pm$ 0.035 \\
Dosage ratio                      & 0.3444 & 0.3322 $\pm$ 0.027 \\
PV (cm$^3$\,g$^{-1}$)            & 0.1081 & 0.1443 $\pm$ 0.015 \\
MPD (nm)                          & 0.1066 & 0.0785 $\pm$ 0.015 \\
Temperature ($^\circ$C)           & 0.0400 & 0.0646 $\pm$ 0.013 \\
pH                                & 0.0098 & 0.0290 $\pm$ 0.007 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Impurity vs.\ permutation importance.}
The top two features (SSA, Dosage ratio) are consistent across methods
and together account for 68\,\% of permutation importance.  PV is
elevated in permutation importance (0.144 vs.\ 0.108 impurity), while
MPD is reduced (0.079 vs.\ 0.107), consistent with impurity-based
overweighting of skewed features (MPD skewness $= 3.86$).
...
10-trial mean MAE                    & 212.0 & 139.6 \\
LOO-CV RMSE (original space)         & 368.0 & 220.8 \\
LOO-CV MAE  (original space)         & 239.2 & 149.5 \\
LOO-CV $R^2$                         & 0.514 & 0.838 \\
$k$-NN oracle RMSE                   & 328   & 310 \\
Wilcoxon GB vs RF ($p$)              & 0.784 & 0.313 \\
Heteroscedasticity $\rho$ ($|r|$ vs $y$) & 0.327 & 0.174 \\
\midrule
...
\midrule
SSA importance (permutation)         & 0.284 & 0.351 \\
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{The GB vs.\ RF ranking is not statistically significant
        in either dataset} (Wilcoxon $p = 0.784$ for DS4, $p = 0.313$
        for DS5).  Both tree-based ensembles are plausible choices; the
        GB ranking is consistent but inconclusive.
  ...
  \item \textbf{The learning curve is no longer saturated.}  The steeper
        ...
        $\approx 1{,}124$ in Dataset~4.  \emph{Caveat:} the power-law
        fit is based on 7 points; extrapolation is speculative.
  ...
  \item \textbf{SSA is the single most important feature} (permutation
        importance 0.351, Spearman $\rho = 0.820$), followed by Dosage
        ratio (0.332).  Together they account for 68\,\% of permutation
        importance.

  \item \textbf{Heteroscedasticity is weaker} in Dataset~5
        ($\rho = 0.174$, $p = 0.281$) than in Dataset~4
        ($\rho = 0.327$, $p = 0.025$), suggesting the revised feature
        set produces more homogeneous errors.  However, per-quartile
        analysis still shows Q4 MAE (287) being 2.0$\times$ Q1 MAE (145).
...
Test MAE mean $\pm$ std & $140 \pm 55$ & Less sensitive to tail \\
LOO-CV RMSE (original space) & 220.8 & High-variance estimator \\
LOO-CV MAE  (original space) & 149.5 & Complementary metric \\
LOO-CV R$^2$ (original space) & 0.838 & Unstable for 1-sample folds \\
95\,\% Bootstrap CI (RMSE) & (143, 232) & $\pm$24\,\% of mean \\
10-fold CV RMSE & $221 \pm 154$ & Independent calibration \\
Repeated 5$\times$10 CV RMSE & $228 \pm 136$ & Gold-standard estimate \\
Test set Jaccard similarity & $0.115 \pm 0.091$ & Low overlap, not independence \\
Wilcoxon GB vs RF (RMSE) & $p = 0.313$ & \textbf{Not significant} \\
Wilcoxon GB vs SVR (RMSE) & $p = 0.002$ & Significant \\
Heteroscedasticity ($|r|$ vs.\ $y$) & $\rho = 0.174$, $p = 0.281$ & Not significant \\
Residual normality (Shapiro) & $p = 0.009$ & Non-normal residuals \\
...
Best GB model MAE  (10-trial) & 212 mg\,g$^{-1}$ & Skew-robust metric \\
Best GB model RMSE (LOO-CV) & 368 mg\,g$^{-1}$ & High-variance estimator \\
