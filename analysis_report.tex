...
\midrule
PCA (90\,\% variance threshold) & 5 PCs & 81.5\,\% at 4 PCs \\
Two-Nearest-Neighbour (2NN) & 2.90 & Facco et al.\ (2017) \\
Correlation dimension          & 2.18 & $r=0.992$; linear log-log fit \\
\bottomrule
\end{tabular}
\end{table}
...
Practically, this means that the curse of dimensionality is \emph{not} as
...
We construct a Vietoris--Rips filtration on the standardised feature space
...
The \emph{Nyquist--Shannon sampling theorem} states that a band-limited
...
uncorrelated.  The near-zero nugget ($\gamma(0) \approx 0$) indicates no
...
The $k$-nearest-neighbours regressor evaluated via leave-one-out (LOO)
cross-validation gives a \emph{data-density-limited} upper bound on
...
provides a prioritised sampling strategy.
